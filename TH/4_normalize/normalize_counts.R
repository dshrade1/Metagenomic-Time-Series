...
######################################################################
# Step 1: load required filepaths
######################################################################
# rm(list = ls())
count_dir <- "~/Desktop/Metagenome_TS/3_count"
normalize_dir <- "~/Desktop/Metagenome_TS/4_normalize"
gff_file <- "~/Desktop/Metagenome_TS/2_map/mapping_input/Trout Bog coassembly/3300000553.gff"

######################################################################
# Step 2: Create RPKM dividend (numerator)
######################################################################
setwd(count_dir)
load("mapped_counts.RData") # mapped

# create the RPKM dividend by row-binding the matrices of all features
RPKMdividend <- as.matrix(do.call(rbind,unname(mapped)))
setwd(normalize_dir)
save(RPKMdividend, file="RPKM_dividend.RData")
...
setwd(count_dir)
load("unmapped_counts.RData") # unmapped
load("total_read_counts.RData") # total_reads
setwd(normalize_dir)
...
save(RPKMdivisor, file="RPKM_divisor.RData")
